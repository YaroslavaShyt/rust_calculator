...
fn build_ui() -> impl Widget<CalcState> {
    ...
    let output_label = Label::new(|data: &CalcState, _env: &Env| format!("Result: {}", data.output))
        .with_text_size(20.0);

    let buttons = build_buttons();

    let calculate_button = Button::new("=").on_click(|ctx: &mut EventCtx, data: &mut CalcState, _env: &Env| {
        ctx.submit_command(CALCULATE);
    });

    let clear_button = Button::new("C").on_click(|_ctx: &mut EventCtx, data: &mut CalcState, _env: &Env| {
        data.input.clear();
    });
    ...
    col.add_child(Flex::row().with_child(calculate_button).with_child(clear_button).padding(25.0));
...
fn build_buttons() -> impl Widget<CalcState> {
    ...
    for row in buttons {
        ...
        for &label in &row {
            if !label.is_empty() {
                let button = Button::new(label).on_click(move |_ctx: &mut EventCtx, data: &mut CalcState, _env: &Env| {
